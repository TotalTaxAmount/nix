  ...
  wayland.windowManager.hyprland = {
      ...
      settings = lib.mkMerge [{
        exec-once = [
          ...
          "${pkgs.copyq}"
          "${inputs.hyprland.packages.${system}.hyprland}/bin/hyprctl setcursor ${config.cursor.name} ${builtins.toString config.cursor.size}"
        ] ++ (if "${host}" == "laptop" then [
        ...
        ]++ (if "${host}" == "laptop" then [ # Laptop sepecific
        ...
        ] else if "${host}" =="desktop" then [ # Desktop specific
