...
, csmSupport ? false, seabios ? null, fdSize2MB ? csmSupport, fdSize4MB ? false
, secureBoot ? false, httpSupport ? false, tpmSupport ? false
, tlsSupport ? false, debug ? false
  # Usually, this option is broken, do not use it except if you know what you are
  # doing.
, sourceDebug ? false }:

assert csmSupport -> throw
  "CSM support has been dropped upstream due to security and maintenance concerns. For qemu, seabios is the default and can be used instead.";
...
in edk2.mkDerivation projectDscPath (finalAttrs: {
  pname = "OVMF";
  inherit version;
  ...
  '' else ''
    mkdir -vp $fd/FV
    cp -v $out/FV/OVMF{,_CODE,_VARS}.fd $fd/FV
  '');
